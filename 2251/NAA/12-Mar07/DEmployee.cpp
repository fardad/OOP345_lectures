...
bool Employee::write(std::ofstream& ofs, ofstream& nfs) const {
   nfs.seekp( 0, ios::end );
   std::streampos nameLoc = nfs.tellp();  // 10
   size_t nameLen = strlen(name); // 5
   nfs.write(name, nameLen); 
   // Employee in file = *this (shallow assignment)
...
bool Employee::read(ifstream& ifs, ifstream& nfs) {
   ...
   ifs.read((char*)&nameLoc, sizeof(nameLoc)); // nameloc: 10
   ifs.read((char*)&nameLen, sizeof(nameLen)); // namelen: 5
   if (ifs) {
